...
class LDTT_Enrollment {
    ...
    /**
     * Add progress to a user for a course
     */
    private static function add_user_progress( $user_id, $course_id, $progress_percentage = null ) {
        ...
        // Flatten step IDs
        $lesson_ids = ! empty( $lessons['lessons'] ) ? wp_list_pluck( $lessons['lessons'], 'ID' ) : [];
        $topic_ids  = ! empty( $topics['topics'] )   ? wp_list_pluck( $topics['topics'], 'ID' )   : [];
        $quiz_ids   = ! empty( $quizzes )            ? wp_list_pluck( $quizzes, 'ID' )            : [];

        $all_steps = array_merge( $lesson_ids, $topic_ids, $quiz_ids );

        if ( empty( $all_steps ) ) {
            return false;
        }

        // Sort by post ID to keep order consistent
        sort( $all_steps );
